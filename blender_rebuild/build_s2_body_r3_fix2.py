...
SPEC = importlib.util.spec_from_file_location("nomadhub_s2_r3_fix1", FIX_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"unable to load first R3 integration fix: {FIX_PATH}")
fix1 = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(fix1)
r3 = fix1.r3
builder = fix1.builder
entry = fix1.entry
...
def sweep_clear_cab_door_panel(name, side_sign):
    ...
    old_mesh = obj.data
    mesh = bpy.data.meshes.new(f"{name}_R3_SWEEP_CLEAR_MESH")
